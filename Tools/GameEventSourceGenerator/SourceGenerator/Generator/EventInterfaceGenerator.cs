...
{  
    ...
    {  
        ...
        {  
            ...
            {  
                ...
                // 生成实现类  
                var implementationClassCode = GenerateImplementationClass(interfaceName, interfaceNode, context);  
    ...
    private string GenerateImplementationClass(string interfaceName, InterfaceDeclarationSyntax interfaceNode, GeneratorExecutionContext context)  
    {  
        var semanticModel = context.Compilation.GetSemanticModel(interfaceNode.SyntaxTree);
        var interfaceSymbol = semanticModel.GetDeclaredSymbol(interfaceNode) as INamedTypeSymbol;
        var interfaceTypeName = interfaceSymbol?.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat) ?? interfaceName;
        var sb = new StringBuilder();  
        ...
        sb.AppendLine($"namespace {Definition.NameSpace}");  
        sb.AppendLine($"{{");  
        sb.AppendLine($"    public partial class {interfaceName}_Gen : {interfaceTypeName}");  
        sb.AppendLine("    {");  
        ...
        sb.AppendLine($"             GameEvent.EventMgr.RegWrapInterface<{interfaceTypeName}>(this);");
        sb.AppendLine("        }");  
    ...
    private string GenerateParameters(MethodDeclarationSyntax method, SemanticModel semanticModel)
    {
        return string.Join(", ", method.ParameterList.Parameters.Select(p => 
        {
            ...
            return typeSymbol != null 
                ? $"{typeSymbol.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat)} {p.Identifier}"
                : $"{p.Type} {p.Identifier}";
        }));
    }
}