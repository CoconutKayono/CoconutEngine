...
namespace Procedure
{
    /// <summary>
    /// 流程 => 下载补丁文件。
    /// <para>【流程位置】CreateDownloader 用户确认之后。</para>
    /// <para>【职责】BeginDownload 执行 YooAsset 批量下载；实时刷新 LoadUpdateUI 进度、网速与剩余时间。</para>
    /// <para>【下一流程】成功 → ProcedureDownloadOver；单文件失败 → 弹窗重试 CreateDownloader 或退出。</para>
    /// <para>【术语】DownloadUpdateCallback：下载进度回调；补丁内容含 AB、GameLogic.dll.bytes 等。</para>
    /// </summary>
