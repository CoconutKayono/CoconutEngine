...
namespace Procedure
{
    /// <summary>
    /// 流程 => 初始化 Package。
    /// <para>【流程位置】Splash 之后，资源热更链路第一步。</para>
    /// <para>【职责】调用 YooAsset InitPackage，创建 DefaultPackage 运行时实例；初始化热更阶段多语言 LoadText。</para>
    /// <para>【下一流程】成功 → ProcedureInitResources；失败 → LoadUpdateUI 弹窗重试或退出。</para>
    /// <para>【术语】Package：YooAsset 资源包容器；InitPackage：绑定 PlayMode、缓存目录、内置/远程文件系统。</para>
    /// </summary>
