...
namespace Procedure
{
    /// <summary>
    /// 流程 => 初始化资源清单。
    /// <para>【流程位置】InitPackage 之后，补丁下载或预加载之前。</para>
    /// <para>【职责】RequestPackageVersionAsync 获取远程/本地 Package 版本号；UpdatePackageManifestAsync 同步资源 Manifest。</para>
    /// <para>【下一流程】Editor/Offline → Preload；HostPlayMode 有补丁 → CreateDownloader；WebGL/边玩边下 → Preload。</para>
    /// <para>【术语】PackageVersion：资源包版本号（如 1074）；Manifest：该版本下全部 AB/DLL 的文件列表与哈希。</para>
    /// </summary>
