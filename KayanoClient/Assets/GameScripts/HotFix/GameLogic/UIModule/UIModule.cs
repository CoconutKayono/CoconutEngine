...
namespace GameLogic
{
    ...
    {
        // 私有字段
        private static Transform _instanceRoot = null;          // UI根节点变换
        private bool _enableErrorLog = true;                    // 是否开启错误日志
        private Camera _uiCamera = null;                        // UI专用相机
        ...
        /// 应用屏幕安全区适配（如异形屏）
        /// </summary>
        /// <param name="safeRect">安全区矩形（屏幕坐标系）</param>
        public static void ApplyScreenSafeRect(Rect safeRect)
        {
            ...
            if (rectTrans != null)
            {
                rectTrans.offsetMin = new Vector2(posX, posY); // 锚点状态下的屏幕左下角偏移
                rectTrans.offsetMax = new Vector2(-offsetMaxX, -offsetMaxY); // 锚点状态下的屏幕右上角偏移
            }
        }
