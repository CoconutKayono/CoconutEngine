...
public static class BuildDLLCommand
{
    ...
    {
#if ENABLE_HYBRIDCLR
        var updateSetting = TEngine.Settings.UpdateSetting;
        if (updateSetting == null)
        {
            Debug.LogError("[BuildDLLCommand] UpdateSetting 未找到。请确认 Assets/TEngine/Settings/UpdateSetting.asset 存在，且场景 Settings 组件已绑定。");
            return;
        }

        var target = EditorUserBuildSettings.activeBuildTarget;
        string aotAssembliesSrcDir = SettingsUtil.GetAssembliesPostIl2CppStripDir(target);
        string aotAssembliesDstDir = Application.dataPath + "/" + updateSetting.AssemblyTextAssetPath;

        foreach (var dll in updateSetting.AOTMetaAssemblies)
        {
    ...
    public static void CopyHotUpdateAssembliesToAssetPath()
    {
        ...
        if (updateSetting == null)
        {
            Debug.LogError("[BuildDLLCommand] UpdateSetting 未找到。请确认 Assets/TEngine/Settings/UpdateSetting.asset 存在。");
            return;
        }
        ...
        string hotfixAssembliesDstDir = Application.dataPath + "/" + updateSetting.AssemblyTextAssetPath;
