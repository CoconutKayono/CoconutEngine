...
namespace TEngine
{
    public class Settings : MonoBehaviour
    {
        ...
        public static NetworkSetting NetworkSetting
        {
            get
            {
#if UNITY_EDITOR
                if (Instance == null)
                {
                    string[] guids = UnityEditor.AssetDatabase.FindAssets("t:NetworkSetting");
                    if (guids.Length >= 1)
                    {
                        string path = UnityEditor.AssetDatabase.GUIDToAssetPath(guids[0]);
                        return UnityEditor.AssetDatabase.LoadAssetAtPath<NetworkSetting>(path);
                    }
                }
#endif
                return Instance.networkSetting;
            }
        }
        ...
        public static UpdateSetting UpdateSetting
        {
            ...
            {
#if UNITY_EDITOR
                if (Instance == null || Instance.updateSetting == null)
                {
...
#endif
                return Instance != null ? Instance.updateSetting : null;
            }
        }
    }
}